...
export class UpdatePasswordViewComponent implements OnInit, OnDestroy {
  ...
  state$ = combineLatest([
    ...
    tap((state) => {
      if (state.updateResult === true) {
        this.uiState.endAction(
          state.translations["passwordUpdate.successfulRequest"],
  ...
  ngOnInit() {
    this.passwordUpdateFormGroup.setValidators(
  ...
  onFormSubmitted(userID: number | string) {
    this.setPasswordAndPasswordConfirmationErrors(
    ...
    if (this.passwordUpdateFormGroup.valid) {
      updateUserAction(this.users.store$)(
        ...
        "users",
        userID,
        this.passwordUpdateFormGroup.getRawValue()
      );
    } else {
