...
export class AddmoduleComponent implements OnInit, OnDestroy {
  ...
  state$ = combineLatest([this.modules.state$, this.translations$]).pipe(
    takeUntil(this._destroy$),
  ...
  performingAction$ = combineLatest([
    this.uiState.uiState,
    this.formViewState$,
  ]).pipe(
    map(([state, formViewState]) => ({
      performingAction:
        state.performingAction || formViewState.performingAction,
    }))
  );
    ...
    if (this.selectedModule) {
      for (const [key, value] of Object.entries(
        ...
        if (this.typeHelper.isDefined(this.componentFormGroup.get(key))) {
          if (key === "roles") {
            const roleControl = (
              this.form.controlConfigs as IHTMLFormControl[]
            ).find((cf) => cf.formControlName === "roles") as SelectInput;
            if (
              roleControl &&
              roleControl instanceof SelectInput &&
              this.typeHelper.isDefined(this.selectedModule.roles)
            ) {
              this.componentFormGroup
                .get(key)
                .setValue(this.selectedModule.roles.map((r) => r.id));
            }
            continue;
          }
          this.componentFormGroup
            .get(key)
            .setValue(getJSObjectPropertyValue(this.selectedModule, value));
        }
      }
    }
  }
  ...
  async onFormSubmit() {
    ...
    if (this.componentFormGroup.valid) {
      const obj = this.transformRequestBody(
        this.componentFormGroup.getRawValue()
      );
      if (!this.typeHelper.isDefined(this.selectedModule)) {
        createModuleAction(this.modules.store$)(
          this.client,
          this.form.endpointURL,
          obj
        ); // deserializeSerializedModule
      } else {
        updateModuleAction(this.modules.store$)(
          this.client,
          this.form.endpointURL,
  ...
  private transformRequestBody(body: object | any) {
    const roleControl = (this.form.controlConfigs as IHTMLFormControl[]).find(
      (cf) => cf.formControlName === "roles"
    ) as CheckBoxInput;
    if (
      roleControl &&
      roleControl instanceof CheckBoxInput &&
      this.typeHelper.isDefined(body.roles)
    ) {
      body = Object.assign(body, {
        roles: roleControl.items
          .map((i, index) => {
