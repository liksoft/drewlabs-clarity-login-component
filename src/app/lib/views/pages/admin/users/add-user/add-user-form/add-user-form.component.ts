...
import {
  createUserAction,
  updateUserAction,
} from "src/app/lib/core/auth/core/actions";
...
import { DynamicFormWapperComponent } from "src/app/lib/core/components/dynamic-inputs/angular/components";
...
export class AddUserFormComponent implements OnDestroy {
  @ViewChild("dynamicFormWrapper", { static: false })
  dynamicFormWrapper: DynamicFormWapperComponent;
  roles: { [index: string]: any }[];
  // tslint:disable-next-line: variable-name
  _destroy$ = createSubject();
  @Input() set formgroupConfig(value: {
    formgroup: FormGroup;
    form: IDynamicForm;
  }) {
    if (value) {
      ...
      if (value.form) {
        const roleControl = (
          value.form.controlConfigs as IHTMLFormControl[]
        ).find((config) => config.formControlName === "roles");
        if (roleControl) {
  ...
  selectedUser$ = this.users.state$.pipe(
  ...
  constructor(
    ...
    @Inject("AUTH_DEPARTMENTS_RESOURCE_PATH")
    private departmentsBackendPath: string
  ) {}
          ...
          if (isDefined(formgroup.get(key))) {
            ...
            if (key === "is_affiliated_to_department") {
              formgroup
                .get(key)
                .setValue(
                  isDefined(user.userDetails.departmentID) ||
                    user.userInfo.isManager
                    ? 1
                    : 0
                );
              continue;
            ...
            if (key === "department_id") {
              formgroup
                .get(key)
                .setValue(getJSObjectPropertyValue(user, value));
              continue;
            }
                ...
                .setValue(
                  isDefined(getJSObjectPropertyValue(user, value))
                    ? MomentUtils.parseDate(
                        getJSObjectPropertyValue(user, value) as string,
                        null,
                        "YYYY-MM-DD"
                      )
                    : null
  ...
  onFormSubmit(formgroup: FormGroup, form: IDynamicForm) {
    ...
    if (formgroup.valid) {
      ...
      createUserAction(this.users.store$)(
        this.client,
        form.endpointURL,
        Object.assign(obj, {
          birthdate: obj.birthdate
            ? MomentUtils.parseDate(obj.birthdate, "YYYY-MM-DD")
            : null,
        })
      );
    } else {
      if (formgroup.hasError("Match")) {
        this.setPasswordAndPasswordConfirmationErrors(formgroup, {
          Match: true,
        });
      }
    }
  ...
  onEditSelectedUser(
    ...
    if (formgroup.valid) {
      const obj = formgroup.getRawValue();
      updateUserAction(this.users.store$)(
        this.client,
        form.endpointURL,
        id,
        Object.assign(obj, {
